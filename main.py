...
def check_konlpy_status():
    """KoNLPy 상태 확인 엔드포인트"""
    from parser import _init_okt, _okt, STOPWORDS
    
    # KoNLPy 초기화 시도
    is_active = _init_okt() and _okt is not None
    test_result = None
...
async def analyze_receipt(
    ...
    try:
        ...
        for item in parsed_data.get("items", []):
            ...
            # 가격이 있는 경우 포함 (0인 경우도 증정 항목일 수 있으므로 포함)
            if item_price and str(item_price).strip() != "":
                try:
                    # 가격이 숫자인지 확인
                    price_num = float(str(item_price).replace(",", ""))
                    # 가격이 0 이상이면 포함 (증정 항목도 포함)
                    if price_num >= 0:
                        ...
                        print(f"[경고] 가격이 음수인 품목 제외: {item_name} - 가격: {item_price}")
                except:
                    print(f"[경고] 가격 파싱 실패, 품목 제외: {item_name} - 가격: {item_price}")
            else:
                # 가격이 없는 경우만 제외
                print(f"[경고] 가격이 없는 품목 제외: {item_name} - 가격: {item_price}")
        ...
        print("\n[상품 목록]")
        for idx, item in enumerate(items_cleaned, 1):
            item_name = item.get("name", "")
            item_price = item.get("price", "0")
            item_quantity = item.get("quantity") or "1"
            print(f"  {idx}. {item_name} - {item_price}원 (수량: {item_quantity})")
        print()
